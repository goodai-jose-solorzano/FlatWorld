...
class GeneratedConfig(FlatWorldConfig):
    ...
    def generate(num_food_elements: int, use_blue_food: bool, seed=None):
        ...
        rs = RandomState()
        rs.seed(seed)
        GeneratedConfig.populate_walls(elements, grid, rs)
        GeneratedConfig.populate_food(elements, grid, num_food_elements, use_blue_food, rs)
        return GeneratedConfig(elements, grid, rs.randint(0, 100000))
    ...
    def populate_walls(elements: list, grid: List[list], rs: RandomState):
        ...
        for w in range(num_walls):
            ...
            we_func = GeneratedConfig.random_wall_element_func(rs)
